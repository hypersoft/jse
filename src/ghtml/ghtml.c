...
load_changed (WebKitWebView  *web_view,
  ...
  if (load_event == WEBKIT_LOAD_COMMITTED) {
    char buf[8192];
    sprintf(buf, "Ghtml = {}; nil = null; Ghtml.window = %p; Ghtml.view = %p; Ghtml.parent = %p; Ghtml.attachment = %p; Ghtml.typeHint = %i;", Ghtml.window, Ghtml.view, Ghtml.parent, Ghtml.attachment, Ghtml.type_hint);
...
static void ghtml_start_application(int argc, char * argv[]) {
  ...
  if (Ghtml.set_position) gtk_window_move(Ghtml.window, Ghtml.geometry.x, Ghtml.geometry.y);
