...
Each of the following Value Macros correspond to JST's notion of the specified type. One obtains the type of a `JSTValue` by calling `JSTValueGetType()`. The following macros are explicitly for comparison with such results, but are also viable as an argument to `JSTDeclareConvertor()`.
